...
class ETLPipeline:
    ...
    def process_daily_quotes(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            [
                pl.col("AdjustmentClose")
                .forward_fill()
                .over("Code")
                .alias("AdjustmentClose"),
                ...
                pl.col("AdjustmentOpen")
                .forward_fill()
                .over("Code")
                .alias("AdjustmentOpen"),
                pl.col("AdjustmentHigh")
                .forward_fill()
                .over("Code")
                .alias("AdjustmentHigh"),
                pl.col("AdjustmentLow")
                .forward_fill()
                .over("Code")
                .alias("AdjustmentLow"),
            ]
        ...
        df = df.with_columns(
            [
                ...
                (
                    ...
                    != (pl.col("Date") + pl.duration(days=3)).dt.month()
                ).alias("IsLateMonth"),
            ]
        )
        ...
        df = df.with_columns(
            [
                (
                    (pl.col("AdjustmentClose") - pl.col("PrevClose"))
                    / pl.col("PrevClose")
                ).alias("DailyReturn"),
                (
                    (pl.col("AdjustmentClose") - pl.col("AdjustmentOpen"))
                    / pl.col("AdjustmentOpen")
                ).alias("IntradayReturn"),
                (
                    (pl.col("AdjustmentOpen") - pl.col("PrevClose"))
                    / pl.col("PrevClose")
                ).alias("OvernightReturn"),
            ]
        )
    ...
    def filter_by_fundamentals(
        self, df: pl.DataFrame, listed_info: Optional[List[Dict]] = None
    ) -> pl.DataFrame:
        ...
        if listed_info:
            allowed_codes = [
                info.get("Code") for info in listed_info if info.get("Code")
            ]
            if allowed_codes:
