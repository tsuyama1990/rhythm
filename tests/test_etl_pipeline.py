...
def test_etl_pipeline():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        pipeline = ETLPipeline(
            raw_data_dir=os.path.join(temp_dir, "raw"), processed_data_dir=processed_dir
        )
        ...
        mock_data = [
            ...
            },
            {
                "Date": "2023-01-05",
                "Code": "65990",
                "AdjustmentOpen": 106.0,
                "AdjustmentHigh": 115.0,
                "AdjustmentLow": 100.0,
                "AdjustmentClose": 112.0,
                "AdjustmentVolume": 1200.0,
            },
            {
                "Date": "2023-01-06",
                "Code": "65990",
                "AdjustmentOpen": 110.0,
                "AdjustmentHigh": 112.0,
                "AdjustmentLow": 105.0,
                "AdjustmentClose": 108.0,
                "AdjustmentVolume": 800.0,
            },
        ]
        ...
        assert (
            abs(
                processed_df.filter(pl.col("Date") == pl.date(2023, 1, 4))[
                    "IntradayReturn"
                ][0]
                - 0.05
            )
        ...
        assert (
            abs(
                processed_df.filter(pl.col("Date") == pl.date(2023, 1, 5))[
                    "DailyReturn"
                ][0]
                - 0.0666666
            )
